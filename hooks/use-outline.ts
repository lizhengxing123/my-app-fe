...
function buildTree(
  ...
  tree: OutlineItem[];
  resolved: { element: HTMLHeadingElement; link: string }[];
} {
...
export function useActiveAnchor(
  ...
  const resolvedHeadersRef = useRef<
    { element: HTMLHeadingElement; link: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    // handler sets active link based on resolvedHeadersRef and updates scroll progress
    const setActiveLink = () => {
      const asideEnabled =
        typeof isAsideEnabled === "function"
          ? isAsideEnabled()
          : Boolean(isAsideEnabled);
      if (!asideEnabled) return;

      const scrollY = window.scrollY;
      const innerHeight = window.innerHeight;
      const offsetHeight = document.querySelector(headerContainer)
        ? (document.querySelector(headerContainer) as HTMLElement).offsetHeight
        : 0;
      ...
      const progress =
        offsetHeight <= innerHeight
          ? 1
          : Math.min(1, Math.max(0, scrollY / (offsetHeight - innerHeight)));
      setScrollProgress(scrollY > 0 ? progress : 0);
    ...
    function activateLink(hash: string | null) {
      ...
      if (hash == null) {
        ...
        try {
          ...
          prevActiveLinkRef.current = (container?.querySelector(selector) ??
